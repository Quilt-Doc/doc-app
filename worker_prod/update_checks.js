...
const runUpdateProcedure = async () => {
    ...
    var patchDataList;
    try {
        var newCheckRunObjList = modifiedChecks.map(checkObj => {
    ...
    var requestPromiseList = patchDataList.map(patchDataObj => await installationClient.patch(`/repos/${repositoryObj.fullName}/check-runs/${patchDataObj.githubId}`, patchDataObj.payload));
