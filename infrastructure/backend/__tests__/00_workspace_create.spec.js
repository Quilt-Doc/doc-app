...
beforeAll(async () => {
    try {
        createdRepositoryIds = await utils.initializeRepositories();
    }
    catch (err) {
    ...
    try {
        createdWorkspaceId = await utils.createWorkspace( createdRepositoryIds );
    }
    catch (err) {
...
describe("Create Workspace", () => {
    ...
    test("Root Document should be created", async () => {
        ...
        try {
            documentRetrieveResponse = await backendUserClient.post(`/documents/${global.createdWorkspaceId}/retrieve`, documentRetrieveData);
        }
        catch (err) {
            console.log(`Error retrieving Root Document - workspaceId: ${global.createdWorkspaceId}`);
            throw err;
        }
    ...
    test("UserStats should be created for test User <-> Workspace", async () => {
        ...
        try {
            userStatRetrieveResponse = await backendUserClient.post(`/reporting/${global.createdWorkspaceId}/retrieve_user_stats`, userStatRetrieveData);
        }
        catch (err) {
            console.log(`Error retrieving UserStats - workspaceId, userId: ${global.createdWorkspaceId}, ${process.env.TESTING_USER_ID}`);
            throw err;
        }
