...
const fetchAllBranches = async (repositoryId) => {
    var scrapedBranches;
    try {
        scrapedBranches = await Branch.find({
...
const fetchAllPRs = async (repositoryId) => {
    var scrapedPRs;
    try {
        scrapedPRs = await PullRequest.find({
            repositoryId: ObjectId(repositoryId.toString()),
        })
            .lean()
            .exec();
        ...
        console.log(err);
        throw Error(
            `Couldn't fetch PullRequests - repositoryId: ${repositoryId}`
        );
    }
    return scrapedPRs;
};
...
const fetchAllCommits = async (repositoryId) => {
    var scrapedCommits;
    try {
        scrapedCommits = await Commit.find({
            repositoryId: ObjectId(repositoryId.toString()),
        })
            .lean()
            .exec();
...
describe("Branch Scraping", () => {
    ...
    test("Should have scraped correct number of branches from brodal_queue", async () => {
        ...
        expect(scrapedBranchNum).toEqual(
            brodalQueueCodeObjectData.branchData.numBranches
        );
    });
    test("Should have scraped correct number of branches from doc-app", async () => {
        ...
        expect(scrapedBranchNum).toEqual(
            docAppCodeObjectData.branchData.numBranches
        );
    });
});
...
describe("Commit Scraping", () => {
    ...
    test("Should have scraped correct number of commits from brodal_queue", async () => {
        ...
        expect(scrapedCommitNum).toEqual(
            brodalQueueCodeObjectData.commitData.numCommits
        );
    });
