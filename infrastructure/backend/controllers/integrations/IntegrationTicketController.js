...
createGithubCard = async (req, res) => {
    const { repositoryFullName, cardId, cardNote,
            cardColumnId, cardCreatedAt, cardUpdatedAt, cardContentUrl } = req.body;

    const source = 'github';

    if (!checkValid(repositoryFullName)) return res.json({success: false, error: 'no integrationTicket repositoryFullName provided'});
    if (!checkValid(cardId)) return res.json({success: false, error: 'no integrationTicket cardId provided'});
