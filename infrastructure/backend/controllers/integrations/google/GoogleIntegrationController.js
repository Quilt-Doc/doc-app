...
const GoogleDriveIntegration = require('../../models/integrations_fs/GoogleDriveIntegration');
const ExternalDocument = require('../../models/integrations_fs/ExternalDocument');
const Workspace = require('../../../models/Workspace');

const { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET } = process.env;
const REDIRECT_URL = "http://localhost:3001/api/integrations/connect/google/callback";
...
handleGoogleConnectCallback = async (req, res) => {
    ...
    res.redirect("http://localhost:3000/workspaces/5f9949ddc89f9adeeaf173bf/google_test");
}
...
bulkScrapeGoogleDrive = async (googleDriveIntegration) => {
    const { accessToken, refreshToken, scope, idToken, workspace, user, repositories } = googleDriveIntegration

    console.log("GOOGLE DRIVE INTEGRATION", googleDriveIntegration);

    const tokens = { access_token: accessToken, refresh_token: refreshToken, scope, id_token: idToken };
    ...
    const currentWorkspace = await Workspace.findById(workspace).lean().select('memberUsers').populate('memberUsers');
    const { memberUsers } = currentWorkspace;

    const memberUserFullNames = memberUsers.map(user => `${user.firstName} ${user.lastName}`);
    const memberUserEmails =  memberUsers.map(user => user.email);


    pages.map(page => {
        page.map(file => {
            const { webViewLink, id, mimeType, 
                createdTime, modifiedTime, 
                lastModifyingUser } = file;
            
            let { owners } = file;
            
            owners = owners.filter(owner => owner.permissionId !== lastModifyingUser.permissionId);
            ...
            owners.map(owner => {
                ...
                for (let i = 0; i < memberUsers.length; i++) {
                    ...
                    if (`${memberUser.firstName} ${memberUser.lastName}` === displayName || memberUser.email === emailAddress) {
...
makeGoogleDriveAssociations = async (googleDriveIntegration) => {
    ...
    googleDriveFiles.map(file => {
        if (file.googleDriveMimeType === 'application/vnd.google-apps.document') {
...
makeGoogleDocAssociations = async (googleDriveDocs, googleDriveIntegration) => {
    const { accessToken, refreshToken, scope, idToken } = googleDriveIntegration;
    ...
    googleDocsData.map((docData, i) => {
        const { body, title, headers, footers, list, revisionId, documentId } = docData;
...
getContentType = (namedStyleType, textStyle) => {
    ...
    if ("link" in textStyle) {
        ...
        type = fontSize > 20 ? "HEADING_1" 
            : fontSize > 16 ? "HEADING_2" 
            : fontSize > 14 ? "HEADING_3"
            : type
    } 
