...
const { renderError, clampValue } = require("./common/utils");
...
getBadge = async (req, res) => {
    const { workspaceId, repositoryId } = req.query;

    if (!checkValid(workspaceId))
        return res.json({
            success: false,
            result: "No badge workspaceId provided",
        });
    if (!checkValid(repositoryId))
        return res.json({
            success: false,
            result: "No badge repositoryId provided",
        });
    ...
    try {
        ...
        brokenDocuments = await Document.count({
            workspace: ObjectId(workspaceId),
            status: "invalid",
        });
    } catch (err) {
        return res.json({
            success: false,
            result: "Failed to get Document info",
        });
    }
    ...
    var brokenSnippets;
    try {
        totalSnippets = await Snippet.count({
            workspace: ObjectId(workspaceId),
            repository: ObjectId(repositoryId),
        });
        brokenSnippets = await Snippet.count({
            workspace: ObjectId(workspaceId),
            repository: ObjectId(repositoryId),
            status: "INVALID",
        });
        ...
        return res.json({
            success: false,
            result: "Failed to get Snippet info",
        });
    }
    ...
    try {
        ...
        return res.json({
            success: false,
            result: "Failed to get Unresolved Checks",
        });
    }
    ...
    await logger.info({
        source: "backend-api",
        message: `Creating Badge - workspaceId, repositoryId, stats: ${workspaceId}, ${repositoryId}, ${JSON.stringify(
            stats
        )}`,
        function: "getBadge",
    });
