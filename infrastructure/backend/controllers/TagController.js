...
createTag = async (req, res) => {
    ...
    if (!checkValid(label))
        return res.json({
            success: false,
            error: "createTag Error: label not provided",
        });
    let color;
    ...
    try {
        ...
        return res.json({
            success: false,
            error: "createTag Error: save query failed",
            trace: err,
        });
    }
...
getTag = async (req, res) => {
    ...
    try {
        returnedTag = await Tag.findOne({ _id: tagId, workspace: workspaceId })
            .lean()
            .exec();
    } catch (err) {
        await logger.error({
        ...
        return res.json({
            success: false,
            error: "getTag Error: findById query failed",
            trace: err,
        });
    }
...
editTag = async (req, res) => {
    ...
    let returnedTag;
    try {
        returnedTag = await Tag.findByIdAndUpdate(
            tagId,
            { $set: update },
            { new: true }
        )
        ...
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error findByIdAndUpdate query failed - tagId, update: ${tagId}, ${JSON.stringify(
                update
            )}`,
            function: "editTag",
        });
        return res.json({
            success: false,
            error: "editTag Error: findByIdAndUpdate query failed",
            trace: err,
        });
    }
    ...
    await logger.info({
        source: "backend-api",
        message: `Successfully edited tag - tagId, update: ${tagId}, ${JSON.stringify(
            update
        )}`,
        function: "editTag",
    });
...
deleteTag = async (req, res) => {
    ...
    try {
        deletedTag = await Tag.findByIdAndRemove(tagId)
            .select("_id")
            .lean()
            .exec();
    } catch (err) {
        ...
        return res.json({
            success: false,
            error: "deleteTag Error: findByIdAndRemove query failed",
            trace: err,
        });
    }
...
retrieveTags = async (req, res) => {
    ...
    try {
        ...
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error find query failed - workspaceId, tagIds, label, color: ${workspaceId}, ${JSON.stringify(
                tagIds
            )}, ${label}, ${color}`,
        ...
        return res.json({
            success: false,
            error: "retrieveTags Error: find query failed",
            trace: err,
        });
    }
...
module.exports = { createTag, getTag, editTag, deleteTag, retrieveTags };
