...
getRepositoryFileSafe = async (req, res) => {
    ...
    if (!checkValid(pathInRepo) && !checkValid(referenceId)) {
        return res.json({
            success: false,
            error: "no repo pathInRepo and referenceId provided",
        });
    }
    ...
    try {
        installationClient = await apis.requestInstallationClient(
            installationId
        );
    } catch (err) {
        await logger.error({
        ...
        return res.json({
            success: false,
            error: err,
            alert: "Failed to authenticate with Github API",
        });
    }
    ...
    try {
        ...
        commitResponse = await installationClient.get(
            `/repos/${fullName}/commits/${lastProcessedCommit}`
        );
    } catch (err) {
        await logger.error({
            source: "backend-api",
        ...
        return res.json({
            success: false,
            error: err,
            alert: "Failed to get Commit Object from Github",
        });
    }
    ...
    try {
        ...
        treeResponse = await installationClient.get(
            `/repos/${fullName}/git/trees/${treeSha}?recursive=true`
        );
    } catch (err) {
        await logger.error({
            source: "backend-api",
        ...
        return res.json({
            success: false,
            error: err,
            alert: "Failed to get Tree Object from Github",
        });
    }
    ...
    if (!fileSha) {
        await logger.error({
            source: "backend-api",
            message: Error(
                `Error couldn't find file path in Tree Object - fullName, lastProcessedCommit, pathInRepo, treeSha: ${fullName}, ${lastProcessedCommit}, ${pathInRepo}, ${treeSha}`
            ),
        ...
        return res.json({
            ...
            alert: "Failed to find file path in Tree Object",
        });
    }
    ...
    var blobResponse;
    try {
        blobResponse = await installationClient.get(
            `/repos/${fullName}/git/blobs/${fileSha}`
        );
    } catch (err) {
        await logger.error({
            source: "backend-api",
        ...
        return res.json({
            success: false,
            error: "getRepositoryFile error getting file blob",
        });
    }
    if (!blobResponse.data.hasOwnProperty("content")) {
        await logger.error({
            source: "backend-api",
            message: `Error blob has no 'content' property - fullName, fileSha: ${fullName}, ${fileSha}`,
            function: "getRepositoryFile",
        });
        return res.json({
            success: false,
            error:
                "getRepositoryFile error: provided fileSha did not return a blob",
        });
    }
...
getRepositoryFile = async (req, res) => {
    ...
    var fileResponse;
    try {
        fileResponse = await installationClient.get(
            `/repos/${fullName}/contents/${pathInRepo}`
        );
    } catch (err) {
        await logger.error({
            source: "backend-api",
        ...
        return res.json({
            success: false,
            error: `Error requesting '/repos/fullName/contents' - fullName, pathInRepo: ${fullName}, ${pathInRepo}`,
        });
    }
    ...
    if (!fileResponse.data.hasOwnProperty("sha")) {
        await logger.error({
            source: "backend-api",
            message: `Error provided pathInRepo did not resolve to a file - fullName, pathInRepo: ${fullName}, ${pathInRepo}`,
            function: "getRepositoryFile",
        });
        return res.json({
            success: false,
            error:
                "getRepositoryFile error: provided path did not resolve to a file",
        });
    }
...
getRepository = async (req, res) => {
    ...
    try {
        returnedRepository = await Repository.findById(repositoryId)
            .lean()
            .exec();
    } catch (err) {
        await logger.error({
        ...
        return res.json({
            success: false,
            error: `Error findById Repository - repositoryId: ${repositoryId}`,
            trace: err,
        });
    }
...
deleteRepository = async (req, res) => {
    ...
    try {
        returnedRepository = await Repository.findByIdAndRemove(repositoryId)
            .lean()
            .select("_id")
            .exec();
    } catch (err) {
        ...
        return res.json({
            success: false,
            error: `Error findByIdAndRemove Repository - repositoryId: ${repositoryId}`,
            trace: err,
        });
    }
...
retrieveCreationRepositories = async (req, res) => {
    ...
    if (!checkValid(installationIds))
        return res.json({
            success: false,
            error: "Retrieve Creation Repositories no installationIds provided",
        });

    if (!checkValid(userId))
        return res.json({
            success: false,
            error: "Retrieve Creation Repositories no userId provided",
        });
    ...
    */
    ...
    try {
        availableRepositories = await Repository.find({
        ...
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error finding repositories - installationIds: ${JSON.stringify(
                installationIds
            )}`,
            function: "retrieveCreationRepositories",
        });
        return res.json({
            success: false,
            error: `Error finding repositories - installationIds: ${JSON.stringify(
                installationIds
            )}`,
            trace: err,
        });
    ...
    console.log("AVAILABLE REPOS", availableRepositories);
...
retrieveRepositories = async (req, res) => {
    ...
    try {
        ...
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error find repositories - fullName, fullNames, installationId: ${fullName}, ${JSON.stringify(
                fullNames
            )}, ${installationId}`,
            function: "deleteRepository",
        });
        return res.json({
            success: false,
            error: `Error find repositories - fullName, fullNames, installationId: ${fullName}, ${JSON.stringify(
                fullNames
            )}, ${installationId}`,
            trace: err,
        });
    ...
    returnedRepositories = returnedRepositories.filter((repo) =>
        repositoriesInWorkspace.includes(repo)
    );
...
jobRetrieveRepositories = async (req, res) => {
    ...
    try {
        ...
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error job fetching repositories - fullName, fullNames, installationId: ${fullName}, ${JSON.stringify(
                fullNames
            )}, ${installationId}`,
        ...
        return res.json({
            success: false,
            error: `Error job fetching repositories - fullName, fullNames, installationId: ${fullName}, ${JSON.stringify(
                fullNames
            )}, ${installationId}`,
            trace: err,
        });
...
updateRepository = async (req, res) => {
    ...
    if (!checkValid(fullName))
        return res.json({
            success: false,
            error: "updateRepository: no repository fullName provided",
        });
    if (!checkValid(installationId))
        return res.json({
            success: false,
            error: "updateRepository: no repository installationId provided",
        });

    if (!checkValid(headCommit))
        return res.json({
            success: false,
            error: "updateRepository: no headCommit provided on `push` event",
        });
    if (!checkValid(cloneUrl))
        return res.json({
            success: false,
            error: "updateRepository: no cloneUrl provided on `push` event",
        });

    if (!checkValid(message))
        return res.json({
            success: false,
            error: "updateRepository: no message provided on `push` event",
        });
    if (!checkValid(pusher))
        return res.json({
            success: false,
            error: "updateRepository: no pusher provided on `push` event",
        });

    var repository;

    try {
        repository = await Repository.findOne({ fullName, installationId })
            .lean()
            .exec();
    } catch (err) {
        await logger.error({
        ...
        return res.json({
            success: false,
            error: `Error finding repository fullName, installationId: ${fullName}, ${installationId}`,
        });
    }
    ...
    if (!ref.includes("refs/heads/")) {
        ...
        return res.json({
            success: false,
            error: `Error invalid ref, does not point to a branch - ref: ${ref}`,
        });
    }
    ...
    if (repository.scanned == false) {
        await logger.info({
            source: "backend-api",
            message: `Ignoring update on unscanned repository fullName, installationId: ${fullName}, ${installationId}`,
            function: "updateRepository",
        });

        return res.json({
            success: true,
            result: `Ignoring update on unscanned repository fullName, installationId: ${fullName}, ${installationId}`,
        });
    }
    ...
    runReferencesData[
        "jobType"
    ] = jobConstants.JOB_UPDATE_REFERENCES.toString();
    ...
    try {
        ...
        return res.json({
            success: false,
            error: `Error dispatching update references job on repository fullName, installationId: ${fullName}, ${installationId}`,
        });
    }
...
removeInstallation = async (req, res) => {
    ...
    try {
        installationRepositories = await Repository.find({
            fullName: { $in: repositories },
        })
            .select("_id fullName installationId")
            .lean()
            .exec();
    } catch (err) {
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error finding installationRepositories - installationId, repositories: ${installationId}, ${JSON.stringify(
                repositories
            )}`,
        ...
        return res.json({
            success: false,
            error: `Error finding installationRepositories - installationId, repositories: ${installationId}, ${JSON.stringify(
                repositories
            )}`,
        });
    }

    var deleteIds = installationRepositories.map((repositoryObj) =>
        repositoryObj._id.toString()
    );
    // Delete all Repositories installation as installed was on
    try {
        await Repository.deleteMany({
            _id: { $in: deleteIds.map((id) => ObjectId(id)) },
        });
    } catch (err) {
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error deleting Repositories - repositories: ${JSON.stringify(
                deleteIds
            )}`,
            function: "removeInstallation",
        });
        return res.json({
            success: false,
            error: `Error deleting Repositories - repositories: ${JSON.stringify(
                deleteIds
            )}`,
        });
    }
    ...
    try {
        await Reference.deleteMany({
            repository: { $in: deleteIds.map((id) => ObjectId(id)) },
        });
    } catch (err) {
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error deleting References - repositoryIds: ${JSON.stringify(
                deleteIds
            )}`,
            function: "removeInstallation",
        });
        return res.json({
            success: false,
            error: `Error deleting References - repositoryIds: ${JSON.stringify(
                deleteIds
            )}`,
        });
    }
    ...
    try {
        await Workspace.updateMany(
            { repositories: { $in: deleteIds.map((id) => ObjectId(id)) } },
            { $pull: { repositories: deleteIds.map((id) => ObjectId(id)) } }
        );
    } catch (err) {
        await logger.error({
            source: "backend-api",
            message: err,
            errorDescription: `Error Removing Repository Ids from Workspace - repositoryIds: ${JSON.stringify(
                deleteIds
            )}`,
            function: "removeInstallation",
        });
        return res.json({
            success: false,
            error: `Error Removing Repository Ids from Workspace - repositoryIds: ${JSON.stringify(
                deleteIds
            )}`,
        });
    }
    ...
    await logger.info({
        ...
        message: `Successfully Deleted ${
            deleteIds.length
        } Repositories - repositoryIds: ${JSON.stringify(deleteIds)}`,
        function: "removeInstallation",
    });
