...
sendInvite = async (req, res) => {
    ...
    try {
        ...
        return res.json({
            success: false,
            error: `Error User findOne query failed - email: ${email}`,
        });
    }
    ...
    if (userWithEmail) {
        try {
            updatedWorkspace = await Workspace.findByIdAndUpdate(
                ...
                {
                    $push: {
                        memberUsers: ObjectId(userWithEmail._id.toString()),
                    },
                },
                { new: true }
            )
                .select("memberUsers _id name")
                .populate("memberUsers");
            addedToWorkspace = true;
        } catch (err) {
            return res.json({
                success: false,
                error: `Error Workspace findOneAndUpdate query failed - workspaceId, userId: ${workspaceId}, ${userWithEmail._id.toString()}`,
            });
        }

        // Update User's workspaces array
        try {
            userWithEmail = await User.findByIdAndUpdate(
                userWithEmail._id.toString(),
                {
                    $push: {
                        workspaces: ObjectId(updatedWorkspace._id.toString()),
                    },
                }
            )
                .lean()
                .select("workspaces _id");
        } catch (err) {
            await logger.error({
                source: "backend-api",
                message: err,
        ...
        try {
            await NotificationController.createAddedNotifications(
                notificationData
            );
        } catch (err) {
    ...
    // If a User doesn't exist, create an invitation object
        ...
        try {
            ...
            return res.json({
                success: false,
                error: `Error saving WorkspaceInvite - workspaceId, email: ${workspaceId}, ${email}`,
            });
        }
    }
    ...
    if (addedToWorkspace) {
        return res.json({
            success: true,
            result: { workspace: updatedWorkspace, user: userWithEmail },
        });
    } else {
        return res.json({ success: true, result: { invite: workspaceInvite } });
    }
};

retrieveInvites = async (req, res) => {
    const { workspaceId } = req.params;

    let invites;

    try {
        invites = await WorkspaceInvite.find({ workspace: workspaceId })
            .lean()
            .exec();
    } catch (e) {
        ...
        return res.json({
            success: false,
            error: "retrieveInvites: find query failed.",
            trace: e,
        });
    }

    return res.json({ success: true, result: invites });
};
