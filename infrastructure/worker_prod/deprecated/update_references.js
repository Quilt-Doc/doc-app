...
const { runSnippetValidation } = require('../jobs/update_snippets');
...
const validateDirectories = async (repoId, workspaceId, repoDiskPath, headCommit, worker, session) => {
    ...
    try {
        ...
        await worker.send({
            action: 'log', info: {
                level: 'error', message: serializeError(err), errorDescription: `Error running 'tree' command on repository ${repoId}`,
    ...
    try {
        ...
        await worker.send({
            action: 'log', info: {
                level: 'error', message: serializeError(err), errorDescription: `Error fetching 'dir' References on repository ${repoId}`,
                source: 'worker-instance', function: 'validateDirectories'
            }
        });
    ...
    if (bulkDirectoryInvalidateOps.length > 0) {
        try {
            ...
            worker.send({
                action: 'log', info: {
                    level: 'info', message: `bulk 'dir' Reference invalidate results: ${JSON.stringify(bulkResult)}`,
                    source: 'worker-instance', function: 'validateDirectories'
                }
            })
        }
            ...
            await worker.send({
                action: 'log', info: {
                    level: 'error', message: serializeError(err), errorDescription: `Error bulk invalidating 'dir' References on repository: ${repoId}`,
                    source: 'worker-instance', function: 'validateDirectories'
                }
            });
    ...
    try {
        ...
        await worker.send({
            action: 'log', info: {
                level: 'error', message: serializeError(err),
    ...
    createdDirectories = createdDirectories.map(dirPath => {
        ...
        return {
            name: dirName, path: dirPath, kind: 'dir',
    ...
    if (createdDirectories.length > 0) {
        try {
            ...
            worker.send({
                action: 'log', info: {
                    level: 'info', message: `New 'dir' Reference bulk insert results repository: ${repoId}\n${JSON.stringify(Object.values(insertResults.insertedIds))}`,
                    source: 'worker-instance', function: 'validateDirectories'
                }
            });
            ...
            await worker.send({
                action: 'log', info: {
                    level: 'error', message: serializeError(err), errorDescription: `Error bulk inserting new 'dir' References on repository: ${repoId}`,
                    source: 'worker-instance', function: 'validateDirectories'
                }
            });
...
const runUpdateProcedure = async () => {
    ...
    await worker.send({
        action: 'log', info: {
            level: 'debug', message: `cloneUrl: ${process.env.cloneUrl}`,
    ...
    try {
        await session.withTransaction(async () => {
            ...
            await worker.send({
                action: 'log', info: {
                    level: 'info', message: `Using repoCommit..headCommit: ${repoCommit}..${headCommit}`,
                    source: 'worker-instance', function: 'runUpdateProcedure'
                }
            });
            // Clone the Repository
            ...
            worker.send({
                action: 'log', info: {
                    level: 'info', message: `repoId, repoDiskPath: ${repoId}, ${repoDiskPath}`,
                    source: 'worker-instance', function: 'runUpdateProcedure'
                }
            });
            ...
            */
            ...
            await worker.send({
                action: 'log', info: {
                    level: 'info', message: ` - repoId, trackedFiles: ${repoId}\n${JSON.stringify(trackedFiles)}`,
                    source: 'worker-instance', function: 'runUpdateProcedure'
                }
            });
            ...
            try {
                ...
                await worker.send({
                    action: 'log', info: {
                        level: 'error', message: serializeError(err), errorDescription: `Error calling runSnippetValidation on repository: ${repoId}`,
            ...
            try {
                ...
                await worker.send({
                    action: 'log', info: {
                        level: 'error', message: serializeError(err), errorDescription: `Error running validateDirectories for cloneUrl: ${process.env.cloneUrl}`,
            ...
            worker.send({
                action: 'log', info: {
                    level: 'info', message: `repoId, fileReferencesModified: ${repoId}, ${JSON.stringify(fileReferencesModified)}`,
                    source: 'worker-instance', function: 'runUpdateProcedure'
                }
            });
            ...
            */
            ...
            if (fileReferencesToCreate.length > 0) {
                ...
                try {
                    ...
                    worker.send({
                        action: 'log', info: {
                            level: 'info', message: `Insert results for 'file' References on repository: ${repoId}\n${JSON.stringify(Object.values(insertResults.insertedIds))}`,
                            source: 'worker-instance', function: 'runUpdateProcedure'
                        }
                    })
                }
                    ...
                    await worker.send({
                        action: 'log', info: {
                            level: 'error', message: serializeError(err), errorDescription: `Error creating new 'file' References on repository: ${repoId}`,
                            source: 'worker-instance', function: 'runUpdateProcedure',
                        }
                    });
            ...
            if (fileReferencesToDelete.length > 0) {
                ...
                try {
                    brokenReferences = await Reference.find({
                        repository: repoId, status: 'valid', kind: 'file',
                    ...
                    await worker.send({
                        action: 'log', info: {
                            level: 'error', message: serializeError(err), errorDescription: `Error fetching 'file' References to invalidate on repository: ${repoId}`,
                            source: 'worker-instance', function: 'runUpdateProcedure',
                        }
                    });
                    transactionAborted = true;
                ...
                for (i = 0; i < brokenReferences.length; i++) {
                    ...
                    refUpdateData.push({
                        _id: currentSearchReference._id, status: 'invalid',
            ...
            if (fileReferencesModified.length > 0) {
                ...
                try {
                    modifiedReferences = await Reference.find({
                        repository: repoId, status: 'valid', kind: 'file',
                        path: { $in: fileReferencesModified.map(file => file.oldRef) }
                    }, '_id', { session }).lean().exec();
                }
                catch (err) {
                    await worker.send({
                        action: 'log', info: {
            ...
            if (fileReferencesToUpdate.length > 0) {
                ...
                try {
                    renamedReferences = await Reference.find({
                        repository: repoId, status: 'valid', kind: 'file',
                ...
                for (i = 0; i < renamedReferences.length; i++) {
                    ...
                    // Update Reference with new status and breakCommit sha
                    refUpdateData.push({
                        _id: currentSearchReference._id, status: 'valid',
                        name: fileName,
                ...
                if (bulkReferenceRenameOps.length > 0) {
                    try {
                        ...
                        await worker.send({
                            action: 'log', info: {
                                level: 'error', message: serializeError(err), errorDescription: `Error renaming 'file' References on repository: ${repoId}`,
                                source: 'worker-instance', function: 'runUpdateProcedure'
                            }
                        });
                        transactionAborted = true;
            ...
            try {
                ...
                await worker.send({
                    action: 'log', info: {
                        level: 'error', message: serializeError(err), errorDescription: `Error deleting repository ${repoId} at:  ${repoDiskPath}`,
                        source: 'worker-instance', function: 'runUpdateProcedure'
                    }
                });
                transactionAborted = true;
            ...
            await worker.send({
                action: 'log', info: {
                    level: 'info', message: `Update Reference Job Success - fullName, headCommit: ${repoObj.fullName}, ${headCommit}`,
                    source: 'worker-instance', function: 'runUpdateProcedure'
                }
            });
        });
