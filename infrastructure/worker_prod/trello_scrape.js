...
const getTrelloIntegrationObj = async (trelloIntegrationId) => {
    ...
    try {
        trelloIntegrationObj = await TrelloIntegration.findById(trelloIntegrationId).lean().exec();
    }
    catch (err) {
        console.log(`err: `);
        console.log(err);
...
bulkScrapeTrello = async () => {
    ...
    var trelloIntegrationObj = await getTrelloIntegrationObj(process.env.trelloIntegrationId);
