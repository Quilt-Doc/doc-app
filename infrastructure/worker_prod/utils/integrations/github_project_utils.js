...
scrapeGithubRepoProjects = async (installationId, repositoryId, installationClient, repositoryObj, workspaceId, worker) => {
    ...
    */
    ...
    try {
        repositoryProjectResponse = await installationClient.get(`/repos/${repositoryObj.fullName}/projects`, {
            headers: {
    ...
    // KARAN TODO: Detect if this error is a 401, 404, 410, etc
        ...
        throw new Error(`Error getting Github repository projects - fullName, installationId: ${repositoryObj.fullName}, ${installationId}`);
    }
    ...
    const bulkGithubProjectInsertList = repositoryProjectList.map((repositoryProjectObjectResponse, idx) => {
        return {
    ...
    if (bulkGithubProjectInsertList.length > 0) {
        ...
        try {
            bulkInsertResult = await GithubProject.insertMany(bulkGithubProjectInsertList, { rawResult: true, });

            newGithubProjectIds = Object.values(bulkInsertResult.insertedIds).map(id => id.toString());


            await worker.send({action: 'log', info: {level: 'info',
                                                        message: `GithubProject insertMany success - bulkInsertResult: ${JSON.stringify(bulkInsertResult)}`,
                                                        source: 'worker-instance',
                                                        function: 'scrapeGithubRepoProjects'}});
        }
        catch(err) {
            await worker.send({action: 'log', info: {level: 'error',
                                                        message: serializeError(err),
                                                        errorDescription: `GithubProject insertMany failed - bulkGithubProjectInsertList: ${JSON.stringify(bulkGithubProjectInsertList)}`,
                                                        source: 'worker-instance',
                                                        function: 'scrapeGithubRepoProjects'}});

            transactionAborted = true;
            transactionError.message = `GithubProject insertMany failed - bulkGithubProjectInsertList: ${JSON.stringify(bulkGithubProjectInsertList)}`;
    ...
    try {
        createdGithubProjects = await GithubProject.find({_id: { $in: newGithubProjectIds.map(id => ObjectId(id)) } }).lean().exec();
    }
    catch (err) {
        await worker.send({action: 'log', info: {level: 'error',
                                                    message: serializeError(err),
                                                    errorDescription: `GithubProject find failed - newGithubProjectIds: ${JSON.stringify(newGithubProjectIds)}`,
                                                    source: 'worker-instance',
                                                    function: 'scrapeGithubRepoProjects'}});

        throw new Error(`GithubProject find failed - newGithubProjectIds: ${JSON.stringify(newGithubProjectIds)}`);
    }
    ...
    var projectColumnsRequestList = createdGithubProjects.map(async (githubProjectObj, idx) => {
        ...
        try {
            // "Accept: application/vnd.github.inertia-preview+json"
            columnListResponse = await installationClient.get(`/projects/${githubProjectObj.projectId}/columns`, {
                headers: {
                  'Accept': 'application/vnd.github.inertia-preview+json'
                }
            });
        ...
        return { columnData: columnListResponse.data, githubProjectId: githubProjectObj._id.toString() };
    });
    ...
    try {
        ...
        await worker.send({action: 'log', info: {level: 'error',
                                                    source: 'worker-instance',
                                                    message: serializeError(err),
                                                    errorDescription: `Error fetching column list - bulkGithubProjectInsertList: ${JSON.stringify(bulkGithubProjectInsertList)}`,
                                                    function: 'scrapeGithubRepoProjects'}});
        throw err;
    }
    ...
    // Non-error responses
    var validResults = columnListResults.filter(resultObj => resultObj.value && !resultObj.value.error);
    ...
    let bulkWriteColumnOps = projectColumnsList.map((projectColumnsData) => {
        var currentColumnUpdate = projectColumnsData.columnData.map(columnObj => columnObj.name);
        var currentColumnIdUpdate = projectColumnsData.columnData.map(columnObj => columnObj.id);
        return ({
            updateOne: {
                filter: { _id: ObjectId(projectColumnsData.githubProjectId.toString()) },
                // Where field is the field you want to update
                update: { $set: { columns: currentColumnUpdate, columnIdList: currentColumnIdUpdate } },
                upsert: false
            }
        })
    })
    ...
    try {
        ...
        await worker.send({action: 'log', info: {level: 'error',
                                                    source: 'worker-instance',
                                                    message: serializeError(err),
                                                    errorDescription: `Error bulk writing columns to GithubProjects - projectColumnsList: ${JSON.stringify(projectColumnsList)}`,
                                                    function: 'scrapeGithubRepoProjects'}});
        throw err;
    }
    ...
    try {
        columnCompleteGithubProjects = await GithubProject.find({ repositoryId: ObjectId(repositoryId.toString()) }).lean().exec();
    }
    catch (err) {
        await worker.send({action: 'log', info: {level: 'error',
                                                    source: 'worker-instance',
                                                    message: serializeError(err),
                                                    errorDescription: `Error finding GithubProjects - repositoryId: ${JSON.stringify(projectColumnsList)}`,
                                                    function: 'scrapeGithubRepoProjects'}});
        throw err;
    }
   ...
   var columnCardRequestList = columnCompleteGithubProjects.map(async (projectObj, idx) => {
        // Create list to use in Promise.all, each element will be used to get all of the cards for one column

        var allCardsInColumnRequestList = projectObj.columns.map(async (columnName, idx2) => {
            ...
            while (!cardResultsExhausted) {
                ...
                try {
                    // "Accept: application/vnd.github.inertia-preview+json"
                    cardPageResponse = await installationClient.get(`/projects/columns/${projectObj.columnIdList[idx2]}/cards?page=${pageNum}&per_page=${perPage}`, {
                        headers: {
                          'Accept': 'application/vnd.github.inertia-preview+json'
                        }
                    });
        ...
        var cardPageResults;
        try {
            cardPageResults = await Promise.allSettled(allCardsInColumnRequestList);
        }
        catch (err) {
            await worker.send({action: 'log', info: {level: 'error',
                                                        source: 'worker-instance',
                                                        message: serializeError(err),
                                                        errorDescription: `Error fetching all cards from column - projectObj: ${JSON.stringify(projectObj)}`,
                                                        function: 'scrapeGithubRepoProjects'}});
            throw err;
        }
        ...
        // Non-error responses
        var validCardResults = cardPageResults.filter(resultObj => resultObj.value && !resultObj.value.error);

        // Error responses
        var invalidCardResults = cardPageResults.filter(resultObj => resultObj.value && resultObj.value.error);
        ...
        return { columnsCardLists: finalCardsInEachColumn, githubProjectObj: projectObj };
    });
    ...
    try {
        allCardsInAllColumnsResults = await Promise.allSettled(columnCardRequestList);
    }
    catch (err) {
        await worker.send({action: 'log', info: {level: 'error',
                                                    source: 'worker-instance',
                                                    message: serializeError(err),
                                                    errorDescription: `Error fetching all cards all GithubProjects - columnCompleteGithubProjects: ${JSON.stringify(columnCompleteGithubProjects)}`,
                                                    function: 'scrapeGithubRepoProjects'}});
        throw err;
    }
    ...
    // Non-error responses
    var validAllCardsInAllColumnsResults = allCardsInAllColumnsResults.filter(resultObj => resultObj.value && !resultObj.value.error);

    // Error responses
    var invalidAllCardsInAllColumnsResults = allCardsInAllColumnsResults.filter(resultObj => resultObj.value && resultObj.value.error);
    ...
    try {
        ...
        await worker.send({action: 'log', info: {level: 'error',
                                                    source: 'worker-instance',
                                                    message: serializeError(err),
                                                    errorDescription: `Error bulk inserting Github Cards - allTicketsToCreate: ${JSON.stringify(allTicketsToCreate)}`,
                                                    function: 'scrapeGithubRepoProjects'}});
