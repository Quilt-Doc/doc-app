...
const getRepositoryObject = async (installationId, repositoryFullName) => {
  ...
  console.log(`InstallationId, repositoryFullName: ${installationId}, ${repositoryFullName}`);
  ...
  try {
    ...
    Sentry.setContext("getRepositoryObject", {
      message: `Error /repositories/job_retrieve call error`,
  ...
  if (!getRepositoryResponse.data.success) {
    ...
    console.log(`Error /repositories/job_retrieve call failed on repositoryFullName: ${repositoryFullName}`);
...
module.exports = {
  getRepositoryObject, filterVendorFiles, parseCommitObjects,
