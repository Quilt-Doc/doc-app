...
const cloneInstallationRepo = async (
  installationId,
  cloneUrlRaw,
  cloneSingleBranch,
  defaultBranch,
  ...
  if (!public) {
    cloneUrl =
      ...
      installToken.value +
      "@" +
      cloneUrlRaw.replace("https://", "");
  } else {
    cloneUrl = `https://${process.env.GITHUB_PUBLIC_USER_NAME}:${
      process.env.GITHUB_PUBLIC_USER_OAUTH
    }@${cloneUrlRaw.replace("https://", "")}`;
  }
...
const ensureRepoCloneCommit = async (repoDiskPath, headCommit) => {
  ...
  try {
    getCurrentCommitResponse = spawnSync("git", ["log", "-1", "--pretty=%H"], {
      cwd: "./" + repoDiskPath,
    });
  } catch (err) {
    console.log(err);
  ...
  if (currentCommitSha != headCommit) {
    ...
    Sentry.captureException(
      Error(`Error cloned repository commit doesn't match headCommit`)
    );
