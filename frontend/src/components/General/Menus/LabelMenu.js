...
class LabelMenu extends React.Component {
    ...
    createTag = () => {
        ...
        this.props.createTag({label: this.state.search}).then((tag) => {
    ...
    renderMarginTop() {
        if (window.innerHeight - this.addButton.offsetTop + this.addButton.offsetHeight > 300) {
            return "-30rem"
        } else {
            return "-5rem"
        }
    ...
    renderListItems(objectLabels){
        return this.props.tags.map((tag, i) => {
            ...
            let border = this.state.position === i ? `1px solid ${this.colors[i]}` : '';
            ...
            return(
                <ListItem 
                    ...
                    shadow = {shadow}
                    color = {this.colors[i]} 
                    backgroundColor = {chroma(this.colors[i]).alpha(0.2)}
                >
                    {tag.label}
                    {labelBool && <ion-icon 
...
export default connect(mapStateToProps, { retrieveTags, createTag })(LabelMenu);
