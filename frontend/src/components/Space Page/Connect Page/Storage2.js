...
class ConnectView extends React.Component {
    ...
    componentDidUpdate = (prevProps, prevState) => {
        ...
        if (prevState.query !== this.state.query) {
            ...
            //console.log("QUERY NOW", this.state.query)
            //console.log("BOOM")
    ...
    renderCards(){
        ...
        return infobankResults.map((result, i) => {
            if (result.isDocument) {
                return (
                    <Card to = {this.renderLink(result)} key = {i} top = {result.image ? false : true}>
                        <Title>
    ...
    render(){
        ...
        return(
            <InfiniteScroll
                pageStart={0}
                loadMore={this.retrieveInfobankResults}
                useWindow = {false}
                hasMore={hasMore}
                initialLoad = {true}
...
export default withRouter(connect(mapStateToProps, { retrieveInfobankResults })(ConnectView));
